...
------------------------------------------------------------------------------
  ...
  Buildfiles:
    ...
    bootstrap.sh
    build-native.sh
      These scripts perform extra build steps. `bootstrap.sh' generates files
      for the native build. `build-native.sh' builds the native code based on
      that setup - you can run this script when you make changes to native code
      and only want to build ghanges. Running `bootstrap.sh' would trigger a
...
2.  Boostrapping
    ...
    Bootstrapping involves creating the `ndk' symlink, running `build-native.sh'
